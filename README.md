...
- IAM access authentication (proving you are who you say you are), by a principal (person, application, device, or process), is done using either Username and Password or Access Keys. (long term credentials)
...
#### IAM Roles - The Tech
- IAM Roles are to be used for temporary access by multiple principals. The role represents a level of access, not you. 
- IAM Roles are assigned either by a Trust policy or a Permissions policy.
- Temporary Security Credentials expire based on time and only have access as specified in the Permissions policy.
- Credentials are generated by STS, Secure Token Service.
  - **sts:AssumeRole** - generates temporary credentials
    - *This might be on the exam. If sts:AssumeRole is mentioned, you'll know roles are involved. If roles or role assumption
      is mentioned, you'll know sts:AssumeRole is somehow involved.*
- 
